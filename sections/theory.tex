...
\par In order to develop our formalism let us consider the set up in \autoref{fig:lens}. For astronomical applications such as precision cosmology the distance from the observer to the lens $D_L$, the distance from the lens to the source $D_{SL}$, and the distance from the observer to the source $D_S$ are much greater than the thickness of the lens along the optical axis. 

We begin by generalizing the results from the previous sections to the case of an extended object. As the name suggests weak lensing operates in the weak gravitational field limit. Therefore, it suffices to work with the newtonian potential of an extended object given by

\begin{equation}
  \psi =  a
  \label{eq:newtpotextend}
\end{equation}


the deflection angle $\vec{\alpha}$ integrate over a newtonian point particle potential with a facto\autoref{fig:lens}
